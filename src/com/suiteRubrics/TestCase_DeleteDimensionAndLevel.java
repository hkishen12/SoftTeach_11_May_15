...
public class TestCase_DeleteDimensionAndLevel extends TestBase
{
	...
	{	 
		TestUtil.checkTestSkip("RubricsSuite","TestCase_DeleteDimensionAndLevel");
	}	 


	@Test
	public void VerifyDeleteDimensionAndLevel() throws Exception
	{
		...
		RubricsUtil.selectRubricDimension(3,3);
		RubricsUtil.clickOnCreateButton();
		RubricsUtil.deleteDimension(3);
		RubricsUtil.deleteLevel(1);
		//check row addition
		if(elementExists("//*[@name='rows[2].cells[0].text']")==null){
			APP_LOGS.debug("Rubric row deleted succesfully!");
			
			//check column addition
			if(elementExists("//*[@id='cell_text_0_0']")==null)
				APP_LOGS.debug("Rubric column deleted succesfully!");
			else
				Assert.fail("Error in deleting rubric level!");

		}else{
			Assert.fail("Error in deleting rubric dimension!");
		}
	}
