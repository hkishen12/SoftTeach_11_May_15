...
public class TestCase_BulkMoveToAnotherFolder extends TestBase
{
	...
	{	 
		TestUtil.checkTestSkip("QuestionsSuite","TestCase_BulkMoveToAnotherFolder");
	}	 
	...
	public void verifyBulkMoveToAnotherFolder() throws Exception
	{
		...
		selectBulkAction.selectByVisibleText("Move to Another Folder");
		WaitForProgressBar();
		...
		WaitForProgressBar();
		waitInSeconds(2);
		elementExists("//*[@id='AddedFolderUI2']/a[.='Move Items']").click();
		WaitForProgressBar();
		if(elementExists(OR.getProperty("ConfirmationMessage")).getText().trim().equals("The selected item(s) have been moved to the folder: ITEMS."))
			APP_LOGS.debug("Questions moved successfully!!");
		else 
			Assert.assertTrue("Error in bulk question movement!", false);
		QuestionUtil.openQuestionFolder("ITEMS");
		boolean flag=false;
		for(int i=1;i<=3;i++){
			if(elementExists("//*[@id='questionsTable']//tr["+i+"]/td[2]").getText().contains(EssayQuestion)){
				APP_LOGS.debug("Questions bulk moved at destination folder!!");
				flag=true;
				break;
			}
		}
		if(!flag){
			Assert.assertTrue("Questions not bulk moved at destination folder!!",false);
		}
	}
